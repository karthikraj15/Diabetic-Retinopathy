...
def segment_file():
	...
	ap.process(uname,uname,class_name,treat,pid,pname,age,gender)
	puti.process(uname)
	mpdf.process(uname)
	return render_template("result.html",class_name="Report_Generated",image_file=path)
...
def upload_file():
    if request.method == 'POST':
        ...
        if class_name=="Mild DR":
                treat="Metformin, Angiotensin-converting enzyme (ACE) \nAnti-VEGF injections - Anti-vascular endothelial growth factor (VEGF) injections. \nAspirin - Help to prevent blood clots.\nSteroids - Such as triamcinolone acetonide, to help reduce inflammation and swelling."
        if class_name=="Moderate DR":
                treat="Corticosteroids \nLaser therapy, also known as photocoagulation, uses a laser to seal off leaking blood vessels in the retina. \nIntravitreal implants - devices that are implanted into the eye. \nSteroids - Such as triamcinolone acetonide, to help reduce inflammation and swelling."
        if class_name=="Severe DR":
                treat="Vitrectomy surgery - vitreous gel inside the eye is removed \nLaser therapy, also known as photocoagulation, uses a laser to seal off leaking blood vessels in the retina. \nIntravitreal implants - devices that are implanted into the eye. \nSteroids - Such as triamcinolone acetonide, to help reduce inflammation and swelling."
        if class_name=="Proliferative DR":
                treat= "Avastin, Fibercept (Evelea) or Ranibizumab (Lucentis) \nLaser therapy, also known as photocoagulation, uses a laser to seal off leaking blood vessels in the retina. \nIntravitreal implants - devices that are implanted into the eye. \nSteroids - Such as triamcinolone acetonide, to help reduce inflammation and swelling."
        if class_name=="No DR":
                treat="Kindly do regular eye test."
        session['treatment']=treat
        return render_template('result.html',class_name=class_name,tret=treat,image_file=image_f)
...
def download_file():
	...
	uname= session['patname']
	print("uname==",uname)
	path='./output/'+str(uname)+'/'+str(uname)+"result.pdf"
