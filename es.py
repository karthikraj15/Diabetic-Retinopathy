...
def process(fname,uname):
    ...
    resized_img = cv2.resize(query, (0, 0), fx=0.5, fy=0.5)
...
def superpixel ( filename,outpath ):
    ...
    cv2.imshow('Feature Extraction',resized_img)
    cv2.waitKey(0)
...
def existingcall(name,outpath, debug, type=None, **options):
   ...
   # ret, markers = cv2.connectedComponents(sure_bg)
    ...
    resized_img = cv2.resize(input_image, (0, 0), fx=0.5, fy=0.5)
    cv2.imshow('Result  watershed algo Image', resized_img)
    cv2.waitKey(0)
...
def ertcall(name,outpath,debug, type=None, **options):
    ...
    gray = cv2.cvtColor(input_image, cv2.COLOR_BGR2GRAY)
    resized_img1 = cv2.resize(gray, (0, 0), fx=0.5, fy=0.5)
    cv2.imshow('Gray Image', resized_img1)
    cv2.waitKey(0)
    cv2.imwrite(outpath+"Gray.png",gray)
    gray = enhance(gray)
    resized_img2 = cv2.resize(gray, (0, 0), fx=0.5, fy=0.5)
    cv2.imshow('Enhance Image Image', resized_img2)
    cv2.waitKey(0)
    cv2.imwrite(outpath+"Enchanced_Gray.png",gray)

    gray_blur = cv2.GaussianBlur(gray, (15, 15), 0)
    resized_img3 = cv2.resize(gray_blur, (0, 0), fx=0.5, fy=0.5)
    cv2.imshow('Gray Blur', resized_img3)
    cv2.waitKey(0)
    cv2.imwrite(outpath+"GrayBlur.png",gray_blur)
    thresh = cv2.adaptiveThreshold(gray_blur, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV, 11, 1)
    resized_img4 = cv2.resize(thresh, (0, 0), fx=0.5, fy=0.5)
    cv2.imshow('Threshold Image', resized_img4)
    cv2.waitKey(0)
    

    kernel = np.ones((3, 3), np.uint8)
    closing = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE,kernel, iterations=4)
    resized_img5 = cv2.resize(thresh, (0, 0), fx=0.5, fy=0.5)
    cv2.imshow('Kernal Image', resized_img5)
    input_image1=input_image
    im = input_image
    im1 = input_image
   ...
   ## cv2.imshow('sure_bg', sure_bg)
    ...
    upper_black = np.array([70, 70, 70], dtype="uint16")
    #black_mask = cv2.inRange(sure_bg, lower_black, upper_black)
    resized_img6 = cv2.resize(sure_bg, (0, 0), fx=0.5, fy=0.5)
    cv2.imshow('Segmented Image', resized_img6)
    cv2.waitKey(0)
   ...
   # ret, markers = cv2.connectedComponents(sure_bg)
    ...
    resized_img8 = cv2.resize(im1, (0, 0), fx=0.5, fy=0.5)
    cv2.imshow('Result Image', resized_img8 )
    cv2.waitKey(0)
